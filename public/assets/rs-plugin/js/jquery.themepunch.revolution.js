	...
	function initSlider(container, opt) {
		...
		if (opt.forceFullWidth == "on" && container.closest(".forcefullwidth_wrapper_tp_banner").length == 0) {
			...
			container
				...
				.append('<div class="tp-fullwidth-forcer" style="width:100%;height:' + container.height() + 'px"></div>');
		...
		if (!container.hasClass("revslider-initialised")) {
			...
			container.find(".tp-caption").each(function (i) {
				...
				try {
					if (
						(jQuery(this).data("ytid") != undefined || jQuery(this).find("iframe").attr("src").toLowerCase().indexOf("youtube") > 0) &&
						addedyt == 0
					) {
						addedyt = 1;
				...
				try {
					if (
						(jQuery(this).data("vimeoid") != undefined || jQuery(this).find("iframe").attr("src").toLowerCase().indexOf("vimeo") > 0) &&
						addedvim == 0
					) {
						addedvim = 1;
			...
			try {
				if (opt.hideThumbsUnderResoluition != 0 && opt.navigationType == "thumb") {
					if (opt.hideThumbsUnderResoluition > jQuery(window).width()) jQuery(".tp-bullets").css({ display: "none" });
					else jQuery(".tp-bullets").css({ display: "block" });
				}
			} catch (e) {}
...
********************************/
	...
	var swipeAction = function (container, opt) {
		...
		if (opt.touchenabled == "on") {
			if (opt.drag_block_vertical == true) aps = "none";
	...
	var setSize = function (img, opt) {
		...
		if (opt.fullScreen == "on") opt.sloth = Math.ceil(jQuery(window).height() / opt.slots);
	...
	var prepareSlides = function (container, opt) {
		...
		container
			.find(">ul:first")
			.css({ overflow: "hidden", width: "100%", height: "100%", maxHeight: container.parent().css("maxHeight") })
		...
		container.find(">ul:first >li >img").each(function (j) {
			...
			img.wrap(
				'<div class="slotholder" style="width:100%;height:100%;"' +
					...
					'"' +
					'data-rotationstart="' +
					img.data("rotationstart") +
					'"' +
					'data-rotationend="' +
					img.data("rotationend") +
					'"' +
					'data-ease="' +
					img.data("ease") +
					'"' +
	...
	var prepareOneSlide = function (slotholder, opt, visible, vorh) {
		...
		else if (img.data("curscale") != undefined) scalestart = img.data("curscale");
					...
					for (var i = 0; i < opt.slots; i++) {
						...
						if (isIE(8)) {
							sh.find(".slot ")
								.last()
								.find(".slotslide")
								.append('<img class="ieeightfallbackimage" src="' + imgsrc + '" style="width:100%;height:auto">');
							ieimgposition(sh, opt);
						}
					}
				} else {
	...
	var ieimgposition = function (nextsh, opt) {
		if (isIE(8)) {
			...
			if (opt.startwidth / opt.startheight < nextsh.data("owidth") / nextsh.data("oheight")) ie8img.css({ width: "auto", height: "100%" });
	...
	var swapSlide = function (container, opt) {
		...
		if (defimg.data("lazyload") != undefined && defimg.data("lazyload") != "undefined" && defimg.data("lazydone") != 1) {
			if (!isIE(8)) defimg.css({ backgroundImage: 'url("' + nextli.find(".defaultimg").data("lazyload") + '")' });
			else {
	...
	var slideAnimation = function (container, opt, nextli, actli, actsh, nextsh) {
		...
		if (nextli.data("randomtransition") == "on") curtransid = Math.round(Math.random() * transtext.length);
		...
		if (nextsh.data("kenburns") == "on") {
			if (
				...
				comingtransition == 0 ||
				comingtransition == "boxfade" ||
				comingtransition == 1 ||
				comingtransition == "papercut" ||
				comingtransition == 16
			)
				comingtransition = 11;
		...
		if (nextli.data("slotamount") == undefined || nextli.data("slotamount") < 1) {
			...
			else if (comingtransition == "flyin") opt.slots = Math.round(Math.random() * 4 + 1);
		} else {
		...
		if (nexttrans == 1) {
			...
			nextsh.find(".slotslide").each(function (j) {
				...
				mtl.add(
					punchgs.TweenLite.from(ss, rand / 1000, { autoAlpha: 0, force3D: "auto", rotation: opt.rotate, ease: punchgs.Power2.easeInOut }),
					rand2 / 1000
				);
			});
		}
		...
		if (nexttrans == 3) {
			...
			actsh.find(".slotslide").each(function () {
				var ss = jQuery(this);
				subtl.add(
					punchgs.TweenLite.to(ss, masterspeed / 1000, { top: opt.sloth, rotation: opt.rotate, force3D: "auto", transformPerspective: 600 }),
					0
				);
				mtl.add(subtl, 0);
			});
			...
			nextsh.find(".slotslide").each(function (i) {
				...
				mtl.add(
					punchgs.TweenLite.fromTo(
						...
						{ autoAlpha: 1, ease: punchgs.Power2.easeInOut, delay: (i * 5) / 1000 }
					),
					0
				);
			});
		...
		if (nexttrans == 12 || nexttrans == 13 || nexttrans == 14 || nexttrans == 15) {
			...
			mtl.add(
				punchgs.TweenLite.from(ssn, speedy, {
					left: twx,
					top: twy,
					scale: fromscale,
					opacity: op,
			...
			if (specials != 1)
				switch (nexttrans) {
					case 12:
						mtl.add(
							punchgs.TweenLite.to(ssa, speedy2, {
								left: 0 - oow + "px",
								force3D: "auto",
								scale: scal,
								opacity: op,
		...
		if (nexttrans == 16) {
			...
			mtl.add(
				punchgs.TweenLite.fromTo(
					...
					{ scale: sc1, rotation: ro1, y: 0 - ooh - ooh / 4, autoAlpha: 0, ease: punchgs.Power2.easeInOut }
				),
				0
			);
			mtl.add(
			...
			nextsh.find(".slotslide").each(function (j) {
				...
				subtl.add(
					punchgs.TweenLite.fromTo(
						...
						{ left: 0, rotationY: 0, top: 0, z: 0, scale: 1, force3D: "auto", rotationX: 0, delay: (j * 50) / 1000, ease: punchgs.Power2.easeInOut }
					),
					0
				);
			...
			nextsh.find(".slotslide").each(function (j) {
				...
				mtl.add(
					punchgs.TweenLite.fromTo(
						...
						{ rotationX: 0, opacity: 1, top: 0, z: 0, scale: 1, rotationY: 0, delay: (j * 50) / 1000, ease: punchgs.Power2.easeInOut }
					),
					0
				);
			...
			actsh.find(".slotslide").each(function (j) {
				...
				mtl.add(
					punchgs.TweenLite.fromTo(
						...
						{ left: 0, rotationX: rot2, top: 0, z: 0, autoAlpha: 1, force3D: "auto", scale: 1, rotationY: rot, ease: punchgs.Power1.easeInOut }
					),
					0
				);
			});
			...
			nextsh.find(".slotslide").each(function (j) {
				...
				mtl.add(
					punchgs.TweenLite.fromTo(
						...
						{ left: 0, rotationX: 0, autoAlpha: 1, top: 0, z: 0, scale: 1, rotationY: 0, delay: (j * 50) / 500, ease: punchgs.Power2.easeInOut }
					),
					0
				);
			});
	...
	var animateTheCaptions = function (nextli, opt, recalled, mtl) {
		...
		allcaptions.each(function (i) {
			...
			if (nextcaption.hasClass("tp-static-layer")) {
				...
				if (nss == -1 || nss == "-1") nextcaption.data("startslide", 0);
			...
			if (handlecaption == 0) {
				...
				if (nextcaption.data("linktoslide") != undefined && !nextcaption.hasClass("hasclicklistener")) {
					...
					if (nextcaption.data("linktoslide") != "no") {
						nextcaption.click(function () {
							...
							} else if (dir == "next") opt.container.parent().find(".tp-rightarrow").click();
							else if (dir == "prev") opt.container.parent().find(".tp-leftarrow").click();
						});
					}
				...
				if (nextcaption.hasClass("tp-videolayer") || nextcaption.find("iframe").length > 0 || nextcaption.find("video").length > 0) {
					...
					if ((videomp != undefined || videowebm != undefined) && nextcaption.find("video").length == 0) {
						...
						var apptxt =
							'<video style="visible:hidden" class="" ' + videoloop + ' preload="' + videopreload + '" width="' + vidw + '" height="' + vidh + '"';
					...
					nextcaption.find("iframe").each(function () {
						...
						if (
							...
							nextcaption.data("videoposter").length > 2 &&
							nextcaption.data("autoplay") != true &&
							!internrecalled
						) {
							if (nextcaption.find(".tp-thumb-image").length == 0)
								nextcaption.append(
									'<div class="tp-thumb-image" style="cursor:pointer; position:absolute;top:0px;left:0px;width:100%;height:100%;background-image:url(' +
										nextcaption.data("videoposter") +
										'); background-size:cover"></div>'
								);
							else punchgs.TweenLite.set(nextcaption.find(".tp-thumb-image"), { autoAlpha: 1 });
						}
				...
				if (!internrecalled) {
					...
					if (
						nextcaption.data("splitin") == "chars" ||
						nextcaption.data("splitin") == "words" ||
						nextcaption.data("splitin") == "lines" ||
						nextcaption.data("splitout") == "chars" ||
						nextcaption.data("splitout") == "words" ||
						nextcaption.data("splitout") == "lines"
					) {
						if (nextcaption.find("a").length > 0)
							nextcaption.data(
								"mySplitText",
								new punchgs.SplitText(nextcaption.find("a"), {
							...
							nextcaption.data(
								"mySplitText",
								new punchgs.SplitText(nextcaption.find(".tp-layer-inner-rotation"), {
									type: "lines,words,chars",
									charsClass: "tp-splitted",
									wordsClass: "tp-splitted",
									linesClass: "tp-splitted",
							...
							nextcaption.data(
								"mySplitText",
								new punchgs.SplitText(nextcaption, {
									type: "lines,words,chars",
									charsClass: "tp-splitted",
									wordsClass: "tp-splitted",
									linesClass: "tp-splitted",
					...
					if (
						...
						ncc.match("sfl") ||
						ncc.match("sfb") ||
						ncc.match("skewfromleftshort") ||
						ncc.match("sfr") ||
						ncc.match("skewfromrightshort")
					)
						frm.opacity = 0;
	...
	var endMoveCaption = function (nextcaption, opt, mdelay, backwards, frame, downscale) {
		...
		if (
			...
			if (ncc.match("ltr") || ncc.match("skewtoright")) frm.x = opt.width + 60;
			else if (ncc.match("ltl") || ncc.match("skewtoleft")) frm.x = 0 - (opt.width + 60);
			else if (ncc.match("ltt")) frm.y = 0 - (opt.height + 60);
			else if (ncc.match("ltb")) frm.y = opt.height + 60;
		...
		if (opt.slideamount > 1 && !(opt.stopAfterLoops == 0 && opt.stopAtSlide == 1)) {
			...
			container.on("starttimer", function () {
				if (
					...
					if ((opt.stopLoop == "on" && opt.next == opt.lastslidetoshow - 1) || opt.noloopanymore == 1) opt.noloopanymore = 1;
					else {
				...
				if ((opt.stopLoop == "on" && opt.next == opt.lastslidetoshow - 1) || opt.noloopanymore == 1) opt.noloopanymore = 1;
					...
					bt.data(
						...
						punchgs.TweenLite.fromTo(
							...
							opt.delay / 1000,
							{ width: "0%" },
							{ force3D: "auto", width: "100%", ease: punchgs.Linear.easeNone, onComplete: countDownNext, delay: 1 }
						)
					);
				}
				if (opt.hideTimerBar == "on") bt.css({ visibility: "hidden" });
			...
			if (prepareonly) {
				...
				defimg.data(
					...
					punchgs.TweenLite.fromTo(
						kbimg,
						dur,
						{ autoAlpha: 1, force3D: punchgs.force3d, transformOrigin: "0% 0%", top: 0, left: 0, scale: imgs.w, x: imgs.x, y: imgs.y },
						{
							autoAlpha: 1,
							rotationZ: roe,
							ease: easeme,
	...
	**************************************************/
				...
				switch (event.type) {
					case "mousemove":
						...
						currslide.find(".tp-parallax-container").each(function () {
							...
							else punchgs.TweenLite.to(pc, 0.4, { force3D: "auto", x: offsh, y: offsv, ease: punchgs.Power3.easeOut, overwrite: "all" });
						});
					...
					case "mouseleave":
						currslide.find(".tp-parallax-container").each(function () {
							...
							else punchgs.TweenLite.to(pc, 1.5, { force3D: "auto", x: 0, y: 0, ease: punchgs.Power3.easeOut });
						});
						break;
				}
			});
	...
	var createThumbs = function (container, opt) {
		...
		if (opt.navigationType == "thumb" || opt.navsecond == "both") {
			cap.append(
				'<div class="tp-bullets tp-thumbs ' + opt.navigationStyle + '"><div class="tp-mask"><div class="tp-thumbcontainer"></div></div></div>'
			);
		}
